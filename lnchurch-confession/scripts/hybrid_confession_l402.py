...
def main():
    ap = argparse.ArgumentParser(description='Execute LN Church Hybrid Confession over L402 using lnd REST.')
    ap.add_argument('--agent-id', required=True)
    ap.add_argument('--message', required=True)
    ap.add_argument('--tier', choices=['light', 'full'], default='light')
    ap.add_argument('--has-relation', action='store_true')
    ap.add_argument('--has-intent-gap', action='store_true')
    ap.add_argument('--automation-level', default='high')
    ap.add_argument('--probe-source', default='openclaw-hybrid-confession-skill')
    ...
    ap.add_argument('--lnd-rest-url', default='https://localhost:8080')
    ap.add_argument('--lnd-tls-cert', default=str(Path.home() / '.lnd/tls.cert'))
    ap.add_argument('--lnd-macaroon-file', default=str(Path.home() / '.lnd/data/chain/bitcoin/mainnet/admin.macaroon'))
    ap.add_argument('--show-sensitive', action='store_true')
    ap.add_argument('--insecure', action='store_true', help='Disable TLS verification for LN Church API only.')
    args = ap.parse_args()
    ...
    probe1 = get_json(session, f'{api_base}/api/agent/probe', params={'src': args.probe_source, 'agentId': args.agent_id}, verify=verify_api)
    probe2 = get_json(session, probe1['next_action']['url'], verify=verify_api)
    ...
    payload = {
        ...
        'tier': args.tier,
        'aiContext': {
            'hasRelation': args.has_relation,
            'hasIntentGap': args.has_intent_gap,
            'automationLevel': args.automation_level,
        },
        'scheme': 'L402',
        'asset': 'SATS',
    }
    ...
    initial = post(session, f'{api_base}/api/agent/hybrid/confession', headers=base_headers, json=payload, verify=verify_api)
    ...
    final = post(session, f'{api_base}/api/agent/hybrid/confession', headers=settled_headers, json=payload, verify=verify_api)
