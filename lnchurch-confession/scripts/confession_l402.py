...
def main():
    ap = argparse.ArgumentParser(description='Execute LN Church Confession over L402 using lnd REST.')
    ap.add_argument('--agent-id', required=True)
    ap.add_argument('--raw-message', required=True)
    ap.add_argument('--probe-source', default='openclaw-confession-skill')
    ...
    initial = post(session, f'{api_base}/api/agent/confession', headers=base_headers, json=payload, verify=verify_api)
    ...
    final = post(session, f'{api_base}/api/agent/confession', headers=settled_headers, json=payload, verify=verify_api)
